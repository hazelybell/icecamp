...
!   integer,parameter :: dp = kind(1.0d0)
  ...
  integer, parameter ::  nr_b_wne = 2*n+m-2
  integer :: b_nw, b_ne, b_sw, b_se
  integer, dimension(m-2) :: b_n, b_s
  integer, dimension(n-2) :: b_e, b_w
  integer, dimension(nr_b_wne) :: b_wne
  integer, dimension(m) :: b_s_all
  ...
  real(dp), dimension(linear_order) :: F, Fb, U, U_prev
  ...
  real(dp), parameter :: approx_ice_visc = 25d6 ! MPa a
  real(dp) :: ice_visc, du_dx, dv_dx, du_dy, dv_dy, nl_resid
  real(dp), parameter :: seawater_density = 1028d0 ! kg m-3
  real(dp), parameter :: initial_thickness = 500d0 ! m
  real(dp), parameter :: domain_ns = -2d5 !m
  real(dp), parameter :: domain_ew = -2d5 !m
! For ssa, the BMP is:
! 1 12
! 0  2
  integer, dimension(nr_v,nr_v), parameter :: bmp = reshape((/1, 21, 12, 2/), shape(bmp)) ! Block Matrix Pattern
! This must be multiplied by the number of non-zero elements in the BMP
  integer :: vi, vj ! For loops over variables
  real(dp), parameter :: flow_law_exponent = 3.0d0
  real(dp), parameter :: flow_law_A = 5.7d-18
  real(dp), parameter :: flow_law_B = flow_law_A**(-1.0/3.0)
...
! Make grid
  do ni = 0, (n-1) ! rows
    do mi = 0, (m-1) ! cols
      p(ni*(m)+mi+1,:) = (/(dble(mi)/dble(m-1))*domain_ew,(dble(ni)/dble(n-1))*domain_ns/)
    end do
  end do
!   do e = 1, nr_p
!     print *, p(e,:)
...
!       print *, "Area", Area
      ...
      if (nliter .eq. 1) then
        ...
        dv_dx = U(VPtoI(2, t(e,1))) * C(3,1) + U(VPtoI(2, t(e,2))) * C(3,2) + U(VPtoI(2, t(e,3))) * C(3,3)
        ice_visc = flow_law_B / (2.0d0 * ((du_dx**2.0d0 + dv_dy**2.0d0 + ((du_dy + dv_dx)**2.0d0)/4.0d0 + du_dx*dv_dy)**(1.0d0/3.0d0)))
!         print *, ice_visc
      end if
      ...
      Ke(1,1) = Area * (4*C(2,1)*C(2,1) + C(3,1)*C(3,1)) * initial_thickness
      Ke(1,2) = Area * (4*C(2,1)*C(2,2) + C(3,1)*C(3,2)) * initial_thickness
      Ke(1,3) = Area * (4*C(2,1)*C(2,3) + C(3,1)*C(3,3)) * initial_thickness
      Ke(2,1) = Area * (4*C(2,2)*C(2,1) + C(3,2)*C(3,1)) * initial_thickness
      Ke(2,2) = Area * (4*C(2,2)*C(2,2) + C(3,2)*C(3,2)) * initial_thickness
      Ke(2,3) = Area * (4*C(2,2)*C(2,3) + C(3,2)*C(3,3)) * initial_thickness
      Ke(3,1) = Area * (4*C(2,3)*C(2,1) + C(3,3)*C(3,1)) * initial_thickness
      Ke(3,2) = Area * (4*C(2,3)*C(2,2) + C(3,3)*C(3,2)) * initial_thickness
      Ke(3,3) = Area * (4*C(2,3)*C(2,3) + C(3,3)*C(3,3)) * initial_thickness
      ...
      do ti = 1, 3
        do tj = 1, 3
            ...
            vK(ki) = vK(ki) - (Ke(ti,tj) * ice_visc)
        end do
      end do
      
      Ke(1,1) = Area * (C(2,1)*C(2,1) + 4*C(3,1)*C(3,1)) * initial_thickness
      Ke(1,2) = Area * (C(2,1)*C(2,2) + 4*C(3,1)*C(3,2)) * initial_thickness
      Ke(1,3) = Area * (C(2,1)*C(2,3) + 4*C(3,1)*C(3,3)) * initial_thickness
      Ke(2,1) = Area * (C(2,2)*C(2,1) + 4*C(3,2)*C(3,1)) * initial_thickness
      Ke(2,2) = Area * (C(2,2)*C(2,2) + 4*C(3,2)*C(3,2)) * initial_thickness
      Ke(2,3) = Area * (C(2,2)*C(2,3) + 4*C(3,2)*C(3,3)) * initial_thickness
      Ke(3,1) = Area * (C(2,3)*C(2,1) + 4*C(3,3)*C(3,1)) * initial_thickness
      Ke(3,2) = Area * (C(2,3)*C(2,2) + 4*C(3,3)*C(3,2)) * initial_thickness
      Ke(3,3) = Area * (C(2,3)*C(2,3) + 4*C(3,3)*C(3,3)) * initial_thickness
      do ti = 1, 3
        do tj = 1, 3
      ...
      Ke(1,1) = Area * (2*C(2,1)*C(3,1) + C(3,1)*C(2,1)) * initial_thickness
      Ke(1,2) = Area * (2*C(2,1)*C(3,2) + C(3,1)*C(2,2)) * initial_thickness
      Ke(1,3) = Area * (2*C(2,1)*C(3,3) + C(3,1)*C(2,3)) * initial_thickness
      Ke(2,1) = Area * (2*C(2,2)*C(3,1) + C(3,2)*C(2,1)) * initial_thickness
      Ke(2,2) = Area * (2*C(2,2)*C(3,2) + C(3,2)*C(2,2)) * initial_thickness
      Ke(2,3) = Area * (2*C(2,2)*C(3,3) + C(3,2)*C(2,3)) * initial_thickness
      Ke(3,1) = Area * (2*C(2,3)*C(3,1) + C(3,3)*C(2,1)) * initial_thickness
      Ke(3,2) = Area * (2*C(2,3)*C(3,2) + C(3,3)*C(2,2)) * initial_thickness
      Ke(3,3) = Area * (2*C(2,3)*C(3,3) + C(3,3)*C(2,3)) * initial_thickness
      do ti = 1, 3
        do tj = 1, 3
      ...
      Ke(1,1) = Area * (2*C(3,1)*C(2,1) + C(2,1)*C(3,1)) * initial_thickness
      Ke(1,2) = Area * (2*C(3,1)*C(2,2) + C(2,1)*C(3,2)) * initial_thickness
      Ke(1,3) = Area * (2*C(3,1)*C(2,3) + C(2,1)*C(3,3)) * initial_thickness
      Ke(2,1) = Area * (2*C(3,2)*C(2,1) + C(2,2)*C(3,1)) * initial_thickness
      Ke(2,2) = Area * (2*C(3,2)*C(2,2) + C(2,2)*C(3,2)) * initial_thickness
      Ke(2,3) = Area * (2*C(3,2)*C(2,3) + C(2,2)*C(3,3)) * initial_thickness
      Ke(3,1) = Area * (2*C(3,3)*C(2,1) + C(2,3)*C(3,1)) * initial_thickness
      Ke(3,2) = Area * (2*C(3,3)*C(2,2) + C(2,3)*C(3,2)) * initial_thickness
      Ke(3,3) = Area * (2*C(3,3)*C(2,3) + C(2,3)*C(3,3)) * initial_thickness
      do ti = 1, 3
        do tj = 1, 3
  ...
  ! BOUNDARY CONDITIONS !!!!!!
    ...
    if (nliter .eq. 1) then
      do bi = 2, m-1
    !     do vi = 1, nr_v
        !F(VPtoI(2, b(bi))) = -2*p(b(bi),2)*(1-p(b(bi),2))-2*p(b(bi),1)*(1-p(b(bi),1))
          F(VPtoI(2, b_s_all(bi))) = F(VPtoI(2, b_s_all(bi))) - (ice_density*accel_grav*initial_thickness/(2.0d0))*(1.0d0-ice_density/seawater_density) \
            * (sqrt((p(b_s_all(bi-1),1)-p(b_s_all(bi),1))**2+((p(b_s_all(bi-1),2)-p(b_s_all(bi),2))**2))/2.0d0 + sqrt((p(b_s_all(bi+1),1)-p(b_s_all(bi),1))**2+((p(b_s_all(bi+1),2)-p(b_s_all(bi),2))**2))/2.0d0) * initial_thickness
    !     end do
      end do
      print *, F(VPtoI(2, b))
    end if
  ...
  !           end if
            if (k_index(VPtoI(vi, b_wne(bi)), VPtoI(vj, ki)) .ne. nr_k_nz+1) then
              vK(k_index(VPtoI(vi, b_wne(bi)), VPtoI(vj, ki))) = 0.0d0
            end if
          end do
        end do
      end do
    ...
    do bi = 1, nr_b_wne
      do vi = 1, nr_v
        vk(k_index(VPtoI(vi, b_wne(bi)), VPtoI(vi, b_wne(bi)))) = 1.0d0
      end do
    end do
  ...
  !   print *, vk(k_index(VPtoI(1, 8),VPtoI(1, 8)))
    ...
    CALL DSDBCG(linear_order, Fb, U, nr_k_nz, iK, jK, vKb, &
      0, 1, 1.0d-5, 100000, iter, converr, ierr, 0, rwork, linear_order * 9, iwork, 100)
    nl_resid = sqrt(sum((U_prev - U)**2.0d0))
    print *, nliter, iter, converr, ierr, nl_resid
    if (nl_resid .le. 1.0d-6) then
      exit
    end if
    do vi = 1, nr_v
...
#define EVALAT(INTEGRAND, WT, CA, CB) (WT * INTEGRAND(coeffs, (CA*domain(1,1) + CB*domain(2,1) + (1-CA-CB)*domain(3,1)), (CA*domain(1,2) + CB*domain(2,2) + (1-CA-CB)*domain(3,2))))
    ...
    real(dp) function int_quad_2b(coeffs, domain)
      real(dp), dimension(3,2), intent(in) :: coeffs
      real(dp), dimension(3,2), intent(in) :: domain
      int_quad_2b = EVALAT(LINTIMESLIN, 0.333333333333333d0, 0.666666666666666d0, 0.166666666666666d0) \
        + EVALAT(LINTIMESLIN, 0.333333333333333d0, 0.166666666666666d0, 0.666666666666666d0) \
        + EVALAT(LINTIMESLIN, 0.333333333333333d0, 0.166666666666666d0, 0.166666666666666d0)
    end function
...
! real(dp) function lintimeslin(coeffs, x, y)
!         real(dp), dimension(3,2), intent(in) :: coeffs
!         real(dp), intent(in) :: x, y
!         lintimeslin = (coeffs(1,1) + coeffs(2,1) * x + coeffs(3,1) * y) &
!           * (coeffs(1,2) + coeffs(2,2) * x + coeffs(3,2) * y)
! end function
