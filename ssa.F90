...
#define ISMIP_SIZE 80d3
    subroutine ismip_hom_c
        implicit none
        integer, parameter :: n = 80, m = 80  ! Grid size n = rows, m = cols
...
!     do ni = 0, (n-1) ! rows
!       do mi = 0, (m-1) ! cols
!         p(ni*(m)+mi+1,:) = (/(dble(ni)/dble(n-1))*domain_ew,(dble(mi)/dble(m-1))*domain_ns/)
!       end do
!     end do
!     ! Make triangle list
!     do ni = 1, n-1 ! rows
...
!       do ni = 1, n-1 ! last column
!         t((m-1)*(n-1)*2+ni,:) = (/((ni-1)*m)+m, ((ni-1)*m)+1, (ni*m)+1/)
!         t((m-1)*(n-1)*2+ni+n-1,:) = (/((ni-1)*m)+m, (ni*m)+1, (ni*m)+m/)
!       end do
!       do mi = 1, m-1 ! last row
!         t((m-1)*(n-1)*2+(2*n-2)+mi,:) = (/((n-1)*m)+mi, ((n-1)*m)+mi+1, (0*m)+mi+1/)
!         t((m-1)*(n-1)*2+(2*n-2)+m-1+mi,:) = (/((n-1)*m)+mi, (0*m)+mi+1, (0*m)+mi/)
!       end do
!       ! final corner
!       t((m-1)*(n-1)*2+(2*n-2)+(2*m-2)+1,:) = (/((n-1)*m)+m, ((n-1)*m)+0+1, (0*m)+0+1/)
!       t((m-1)*(n-1)*2+(2*n-2)+(2*m-2)+2,:) = (/((n-1)*m)+m, (0*m)+0+1, (0*m)+m/)
!     end if
! 
!     print *, "Triangles"
!     do ti = 1, (m-1) * (n-1) * 2
...
subroutine fem_imr(nr_p, p, nr_t, t, bed, thickness, basal_traction_coeff, grounded, nr_b, max_neighbors, nr_calving_front, calving_front, nr_dirichlet, dirichlet, dirichlet_val, ds_dx_fudge, ds_dy_fudge, flow_law_A, x_wrap, y_wrap, x_wrap_extra, y_wrap_extra)
...
!     End Interface
    ...
    max_nliter = (INT(SQRT(DBLE(nr_p))))/4+2
    max_nliter_imr = (INT(SQRT(DBLE(nr_p))))/4+2
    guess = 0d0
    use_guess = 0
...
#define NR_K_NZ ((nr_p + nr_t * 3 + nr_b) * nr_bmp_nz)
    ...
    real(dp) :: ice_visc, du_dx, dv_dx, du_dy, dv_dy
    real(dp), intent(out) :: nl_resid
    real(dp) :: ds_dx, ds_dy ! derivatives of the surface elev., m
    real(dp), intent(in) :: ds_dx_fudge, ds_dy_fudge ! derivatives of the surface elev., m
    real(dp), intent(in) :: x_wrap, y_wrap, x_wrap_extra, y_wrap_extra
...
#else
    real(dp), parameter :: mode_tol = final_tol*10d0 ! all updates w/in 2x10^-11 m/a
#endif
#ifdef LAGRANGEBC
...
!         print *, "Area", Area
        ...
        ice_visc_zeta = ((1d0-flow_law_exponent)/(2d0*flow_law_exponent)) * (flow_law_B / 2d0) * ((flow_law_epsilon + du_dx**2d0 + dv_dy**2d0 + ((du_dy + dv_dx)**2d0)/4d0 + du_dx*dv_dy)**((1d0-flow_law_exponent)/(2d0*flow_law_exponent) - 1d0))

        if (nliter .eq. 1) then
          ice_visc = approx_ice_visc
        else  ! GLEN'S FLOW LAW
  !         print *, du_dx, dv_dy, dv_dx, du_dy
          ice_visc = (flow_law_B / 2d0) * ((flow_law_epsilon + du_dx**2d0 + dv_dy**2d0 + ((du_dy + dv_dx)**2d0)/4d0 + du_dx*dv_dy)**((1d0-flow_law_exponent)/(2d0*flow_law_exponent)))
...
#define SETUP_ELT_SUBMAT(MAT, TERM11, TERM12, TERM21, TERM22, ALL1, ALL2) MAT = reshape((/ \
  ...
  !       print *, ds_dx, ds_dy
        do ti = 1, 3
          F(VPtoI(1, t(e, ti))) = F(VPtoI(1, t(e, ti))) + 1d0/3d0 * Area * (ice_density * accel_grav * element_thickness(ti) * ds_dx)
          F(VPtoI(2, t(e, ti))) = F(VPtoI(2, t(e, ti))) + 1d0/3d0 * Area * (ice_density * accel_grav * element_thickness(ti) * ds_dy)
...
!       (sqrt((p(calving_front(bi-1),1)-p(calving_front(bi),1))**2+((p(calving_front(bi-1),2)-p(calving_front(bi),2))**2))/2.0d0 + sqrt((p(calving_front(bi+1),1)-p(calving_front(bi),1))**2+((p(calving_front(bi+1),2)-p(calving_front(bi),2))**2))/2.0d0)
  ...
  !   print *, "Variable ", 4
  !   do pi = 1, nr_p
  !     print *, p(pi,1:2), thickness(pi)
  !   end do
  !   print *, "End Variable"
  !   do vi = 1, nr_v
  !     print *, "Variable ", vi+4
  !     do pi = 1, nr_p
  !       print *, p(pi,1:2), F(VPtoI(vi, pi))
  !     end do
  !     print *, "End Variable"
  !   end do
!     print *, "Variable ", 10
!     do pi = 1, nr_p
!       print *, p(pi,1:2), basal_coeff(pi)
!     end do
!     print *, "End Variable"
!     print *, "Variable ", 11
!     do pi = 1, nr_p
!       print *, p(pi,1:2), basal_coeff_diff(pi)
!     end do
!     print *, "End Variable"
!     print *, "Variable ", 12
!     do pi = 1, nr_p
!       print *, p(pi,1:2), basal_traction_coeff(pi)**(-.5d0)
!     end do
!     print *, "End Variable"
...
!       thickness = 1000d0 - bed
        basal_traction_coeff(pi) = (1000d0 + 1000d0 * dsin(p(pi,1)*2d0*pic/ISMIP_SIZE) * dsin(p(pi,2)*2d0*pic/ISMIP_SIZE))**(-2d0)
    end do
    call fem_imr(nr_p, p, nr_t, t, bed, thickness, basal_traction_coeff, grounded, 0, max_neighbors, 0, b, 0, b, b, dtan(2d0*pic/3600d0), 0d0, 1d-16, ISMIP_SIZE * (DFLOAT(m)/DFLOAT(m+1)), ISMIP_SIZE * (DFLOAT(n)/DFLOAT(n+1)), ISMIP_SIZE/m, ISMIP_SIZE/n)
end subroutine